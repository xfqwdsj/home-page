...
interface Profile {
  ...
  'proxy-providers': any;
  proxies: Array<any>;
  'proxy-groups': Array<any>;
  'rule-providers': any;
  rules: Array<string>;
}

export const defaultClashProfile: Profile = {
  port: 7890,
  'socks-port': 7891,
  'redir-port': 7892,
  'allow-lan': false,
  ...
  'proxy-groups': [
    ...
    { name: 'iCloud', type: 'select', proxies: ['DIRECT', 'PROXY'] },
    { name: 'Apple', type: 'select', proxies: ['DIRECT', 'PROXY'] },
    { name: 'Google', type: 'select', proxies: ['PROXY', 'DIRECT'] },
    { name: '中国IP地址', type: 'select', proxies: ['DIRECT', 'PROXY'] },
    { name: '中国GeoIP', type: 'select', proxies: ['DIRECT', 'PROXY'] },
    { name: '私有域名', type: 'select', proxies: ['DIRECT', 'PROXY'] },
    { name: '保留IP地址', type: 'select', proxies: ['DIRECT', 'PROXY'] },
    { name: '本地进程', type: 'select', proxies: ['DIRECT', 'PROXY'] },
    { name: '其他', type: 'select', proxies: ['DIRECT', 'PROXY'] },
    {
      name: '拦截地址',
      type: 'select',
